    ...
    return (
        <Card className="min-w-60 max-w-60 min-h-60 max-h-60 bg-secondary">
            <CardHeader className="flex">
                <h1 className="text-sm">{`Room: ${room.name} Capacity: ${room.capacity}`}</h1>
            </CardHeader>
            <Divider/>
            ...
            <CardFooter  className="flex flex-row-reverse absolute bottom-1">
                <Button color="primary" className="self-end" variant="solid" onClick={() => setSelectedRoom(room)}>Schedule</Button>
            </CardFooter>
        </Card>  
    )
}